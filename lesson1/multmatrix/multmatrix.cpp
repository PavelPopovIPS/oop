...
Matrix MultMatrix(const Matrix& matrixA, const Matrix& matrixB)
{
	Matrix matrixNew;
	...
	for (int k = 0; k < 3; k++)
	{
		...
		// вычесление каждого элемента строки
		{
			...
			for (int i = 0; i < 3; i++)
			// вычесление элемента новой матрицы
			{
				elem += matrixA.pos[k][i] * matrixB.pos[i][j];
			}

			matrixNew.pos[k][j] = elem;
		}
	}
